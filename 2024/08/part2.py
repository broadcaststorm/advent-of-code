...
def calculate_antinodes(locations: list[tuple], grid_size: tuple[int, int]) -> set[tuple]:

    # We need to return a unique set of antinodes that have been found
    antinodes = set()

    # Pick the maximum grid dimension
    iterations = grid_size[0] if grid_size[0] > grid_size[1] else grid_size[1]

    for i, node_a in enumerate(locations):
        for j, node_b in enumerate(locations[i+1:]):
            dx, dy = (node_b[0] - node_a[0], node_b[1] - node_a[1])

            # We need to go multiple nodes along the line
            for n in range(1, iterations):
                b_node = (node_b[0] + dx*n, node_b[1] + dy*n)

                # Still in line and on the grid
            ...
            # We need to go the other direction, multiple nodes along the line
            for n in range(1, iterations):
                b_node = (node_b[0] - dx*n, node_b[1] - dy*n)
            ...
            # Let's add node_a and node_b since they have to be harmonic
            # antinodes by definition
